/**
 * \file M3Passivity.h
 * \author Vincent Crocher
 * /brief The <code>M3Passivity</code> class represents a passivity controller state machine
 * \version 0.1
 * \date 2020-11-25
 *
 * \copyright Copyright (c) 2020
 *
 */
#ifndef M3PASSIVITY_SM_H
#define M3PASSIVITY_SM_H

#include <sys/time.h>

#include <array>
#include <cmath>
#include <fstream>
#include <iostream>
#include <string>
#include <csignal> //For raise()

#include "RobotM3.h"
#include "StateMachine.h"
#include "FLNLHelper.h"

// State Classes
#include "M3PassivityStates.h"

/**
 * @brief Example implementation of a StateMachine for the M3Robot class. States should implemented M3DemoState
 *
 */
class M3Passivity : public StateMachine {
   public:
    bool running = false;
    std::chrono::steady_clock::time_point time_init; // initial time that machine started
    double time_running; // time passed after initialisation in [s]
    /**
     *  \todo Pilot Parameters would be set in constructor here
     *
     */
    M3Passivity();
    ~M3Passivity();
    void init();
    void end();

    void hwStateUpdate();

    /**
     * Pointers to the relevant states - initialised in init
     *
     */
    M3CalibState *calibState;
    M3DemoMinJerkPosition* minJerkState;
    M3DemoImpedanceState *impedanceState;


    //PO/PC
    VM3 Energy;
    VM3 Power;


   protected:
    RobotM3 *robot; /*!< Pointer to the Robot*/
    FLNLHelper *UIserver; /*!< Pointer to communication server*/

   private:
    EventObject(EndCalib) * endCalib;
    EventObject(GoToNextState) * goToNextState;
};

#endif /*M3PASSIVITY_SM_H*/
